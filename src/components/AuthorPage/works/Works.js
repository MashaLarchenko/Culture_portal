...
const Works = ({ data }) => {
  ...
  const masterpieces = listOfWorks.map((work, index) => {
    ...
    const style =
      index % 2 ? "rgba(204, 204, 204, 0.5)" : "rgba(255, 255, 255, 0.85)"
    return (
      <TableRow key={index}>
        <TableCell className={classes.cell} style={{ background: style }}>
          {film}
        </TableCell>
        <TableCell className={classes.dateCell} style={{ background: style }}>
          {year}
        </TableCell>
      </TableRow>
    )
  ...
  return (
    <Container className={classes.container} id={"works"}>
      ...
      <Table className={classes.table}>
        ...
        <TableBody>{masterpieces}</TableBody>
      </Table>
    </Container>
  )
}
